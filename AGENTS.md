...
- If a task needs to sync local `main` and `origin/main` to the upstream repo's `main` (non-fork), ask the user to run `just sync` (or `just sync-upstream-main`) locally because the SSH key requires a passphrase. Do this before attempting to merge `main` into `dev`.
...
2. Once those pass, if any changes were made in common, core, or protocol, run the complete test suite with `cargo test --all-features`.
   When running interactively, `just fmt` does not require approval.
...
When the user asks to implement an item from `wants.md`:
