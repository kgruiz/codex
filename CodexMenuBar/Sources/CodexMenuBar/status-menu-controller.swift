...
final class StatusMenuController {
    ...
    func Render(
        ...
        UpdateButton(connectionState: connectionState, runningCount: turns.filter { $0.status == .inProgress }.count)

        menu.removeAllItems()
        let status = NSMenuItem(title: HeaderTitle(connectionState: connectionState, runningCount: turns.filter { $0.status == .inProgress }.count), action: nil, keyEquivalent: "")
        ...
        let reconnect = NSMenuItem(title: "Reconnect app-server", action: #selector(OnReconnect), keyEquivalent: "r")
    ...
    private func TurnTitle(turn: ActiveTurn, animationFrame: Int, now: Date) -> String {
        ...
        return "[\(turn.threadId.prefix(8))/\(turn.turnId)] \(statusLabel) \(elapsed) \(bar)\n  \(legend) - \(label)"
    }
