...
Specify a program to run when approval is needed. Codex appends a single JSON argument describing the approval request.
...
```toml
approval_command = ["python3", "/Users/alice/.codex/notify-approval.py"]
```

### completion_command

Specify a program to run when a turn completes. Codex appends a single JSON argument describing the completed turn.
...
```toml
completion_command = ["python3", "/Users/alice/.codex/notify-complete.py"]
```

### approval_notify

Enable built-in, cross-platform notifications when approvals are requested. These do not require custom scripts.
...
Enable built-in, cross-platform notifications when turns complete.
...
# Show startup tooltips in the welcome screen.
# Defaults to true.
show_tooltips = true

# Ordered list of status line items to show in the footer.
# Defaults: ["model", "context", "cwd", "git-branch"]
...
> [!NOTE]
> `tui.notifications` is built-in and limited to the TUI session. For programmatic or cross-environment notifications, use `approval_command` and `completion_command` to run external programs that receive event JSON. These settings are independent and can be used together.
...
Override TUI keybindings via the `[keybindings]` table. Each action accepts a string (single
binding) or an array of strings. Unknown bindings are ignored with a warning.
...
editor_move_right = "Alt+Right"
```

Supported actions: `submit`, `newline`, `paste`, `copy_prompt`, `editor_move_left`,
`editor_move_right`, `editor_move_up`, `editor_move_down`, `editor_move_word_left`,
`editor_move_word_right`, `editor_delete_backward`, `editor_delete_forward`,
`editor_delete_word_backward`, `editor_delete_word_forward`, `editor_home`, `editor_end`.
...
| `approval_command`                               | array<string>                                                     | External program for approval notifications.                                                                                    |
...
| `keybindings.<action>`                           | string \| array<string>                                           | Override TUI keybindings; values are key chord strings (e.g., `Enter`, `Shift+Enter`).                                          |
