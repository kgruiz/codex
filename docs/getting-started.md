...
#### Switch modes without leaving the composer

Use Alt+P (plan), Alt+A (ask), and Alt+N (normal) to switch modes while typing or while output is running.

#### Run slash commands mid-draft

Start a new line with `/` to open the slash command list and run a command without clearing the rest of your prompt.
